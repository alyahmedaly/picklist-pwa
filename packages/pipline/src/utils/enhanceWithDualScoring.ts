...
import { computeHealthScore, computeHealthGrade } from './computeHealthGrade.ts';


/**
 * Apply percentile ranking and grade assignment
 *
...
export function getScoringStatistics(products: Product[]): {
  ...
  const globalGrades = products
    ...
    .filter((grade): grade is import('../../../../src/data/transform/types.ts').HealthGrade => grade !== undefined);
