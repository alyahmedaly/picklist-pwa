...
import { computeHalalAnalysis } from '../../../src/product/compute/computeHalalAnalysis.ts';
...
describe('computeHalalAnalysis (T002 RED phase)', () => {
  ...
  test('detects haram gelatin sources as haram', () => {
    ...
    for (const source of haramSources) {
      ...
      expect(result.status).toBe('haram');
      expect(result.flags.hasPork).toBe(true);
      // With word boundary matching, compound words like 'varkensgelatine' are detected
      // by their specific compound term, not by substring matching
      if (source === 'varkensgelatine') {
        expect(result.details.problematicIngredients).toContain('varkensgelatine');
  ...
  // Regression tests for word boundary matching fixes
  describe('Word boundary detection regression tests', () => {
    test('should not detect rum in durumtarwegriesmeel (durum wheat semolina)', () => {
      const result = run('Ingrediënten: durumtarwegriesmeel*');
      expect(result.status).toBe('halal');
      expect(result.flags.hasAlcohol).toBe(false);
      expect(result.details.problematicIngredients).toEqual([]);
    });

    test('should detect rum when it appears as a separate word', () => {
      const result = run('suiker, rum, vanille-extract');
      expect(result.status).toBe('haram');
      expect(result.flags.hasAlcohol).toBe(true);
      expect(result.details.problematicIngredients).toContain('rum');
    });

    test('should not flag innocent products with substring matches', () => {
      const falsePosotiveIngredients = [
        { ingredient: 'durumtarwegriesmeel', contains: 'rum' },
        { ingredient: 'hamstring extract', contains: 'ham' },
        { ingredient: 'worshipfruit', contains: 'worst' },
        { ingredient: 'porcelain powder', contains: 'pork' },
      ];

      falsePosotiveIngredients.forEach(({ ingredient, contains }) => {
        const result = run(`water, ${ingredient}, sugar`);
        expect(result.status).not.toBe('haram');
        expect(result.flags.hasAlcohol).toBe(false);
        expect(result.flags.hasPork).toBe(false);
        expect(result.details.problematicIngredients).not.toContain(contains);
      });
    });

    test('should handle AH Biologisch Spaghetti correctly', () => {
      const result = run('Ingrediënten: durumtarwegriesmeel*');
      expect(result.status).toBe('halal');
      expect(result.flags.hasAlcohol).toBe(false);
      expect(result.flags.hasPork).toBe(false);
    ...
    test('should still detect actual problematic ingredients alongside durum wheat', () => {
      const result = run('water, 5% varkensvlees, durumtarwegriesmeel, rum');
      expect(result.status).toBe('haram');
      expect(result.flags.hasAlcohol).toBe(true);
      expect(result.flags.hasPork).toBe(true);
      expect(result.details.problematicIngredients).toContain('varkensvlees');
      expect(result.details.problematicIngredients).toContain('rum');
      expect(result.details.problematicIngredients).not.toContain('durumtarwegriesmeel');
    });

    test('should handle alcohol percentage extraction with word boundaries', () => {
      const result = run('water, rum 40%, sugar');
      expect(result.status).toBe('haram');
      expect(result.flags.hasAlcohol).toBe(true);
      expect(result.details.alcoholContent).toBe(40);
      expect(result.details.problematicIngredients).toContain('rum');
    });

    test('should handle case insensitive matching with word boundaries', () => {
      const result = run('Sugar, RUM, Water');
      expect(result.status).toBe('haram');
      expect(result.flags.hasAlcohol).toBe(true);
      expect(result.details.problematicIngredients).toContain('rum');
    });
